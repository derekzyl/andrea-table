  ...
  return (
    <div className="sm:col-span-7 md:col-span-5 lg:col-span-7">
      <div className=" btn-group">
        ...
        {state.show.deleteButton && state.show.checkBox && (
          <button
            className="btn buttons-pdf buttons-html5 btn-sm"
            onClick={() => {
              const filteredData = state.bodyData.filter(
                (data) => data.checkBox && data.checkBox === true
              );

              if (filteredData.length > 0) {
                data.tableData.fn.deleteFn &&
                  data.tableData.fn.deleteFn(filteredData as any);
              }
            }}
            disabled={
              state.bodyData.filter(
                (data) => data.checkBox && data.checkBox === true
              ).length > 0
                ? false
                : true
